...
static struct io_libecc_Ecc *ecc;
...
int main (int argc, const char * argv[])
{
	...
	ecc = io_libecc_Ecc.create();
	
	io_libecc_Ecc.addFunction(ecc, "alert", alert, -1, 0);
	io_libecc_Ecc.addFunction(ecc, "print", print, -1, 0);
	...
	io_libecc_Ecc.destroy(ecc), ecc = NULL;
...
static int alertUsage (void)
{
	...
	io_libecc_Env.printError(sizeof(error)-1, error, "libecc [<filename> | --test | --test-verbose | --test-quiet]");
...
static void test (const char *func, int line, const char *test, const char *expect, const char *text)
{
	...
	if (testVerbosity > 0 || !setjmp(*io_libecc_Ecc.pushEnv(ecc)))
		io_libecc_Ecc.evalInput(ecc, io_libecc_Input.createFromBytes(test, (uint32_t)strlen(test), "%s:%d", func, line), io_libecc_ecc_stringResult);
	
	if (testVerbosity <= 0)
		io_libecc_Ecc.popEnv(ecc);
	...
	{
		++testErrorCount;
		io_libecc_Env.printColor(io_libecc_env_red, io_libecc_env_bold, "[failure]");
		io_libecc_Env.print(" %s:%d - ", func, line);
		io_libecc_Env.printColor(0, io_libecc_env_bold, "expect \"%s\" was \"%.*s\"", expect, length, bytes);
	...
	if (text)
	{
		...
		struct io_libecc_Input *input = io_libecc_Ecc.findInput(ecc, ecc->text);
		...
		if (!bytes || ecc->text.bytes - bytes != textStart || ecc->text.length != textLength)
		{
			...
			io_libecc_Env.printColor(io_libecc_env_red, io_libecc_env_bold, "[failure]");
			io_libecc_Env.print(" %s:%d - ", func, line);
			io_libecc_Env.printColor(0, io_libecc_env_bold, "text should highlight `%.*s`", textLength, test + textStart);
			io_libecc_Env.newline();
			goto error;
		}
	}
	...
	if (testVerbosity >= 0)
	{
		io_libecc_Env.printColor(io_libecc_env_green, io_libecc_env_bold, "[success]");
		io_libecc_Env.print(" %s:%d", func, line);
		io_libecc_Env.newline();
	}
	
	error:
	io_libecc_Ecc.garbageCollect(ecc);
}
...
//	test("debugger", "undefined", NULL);
	...
	if (testErrorCount)
		io_libecc_Env.printColor(0, io_libecc_env_bold, "test failure: %d", testErrorCount);
	else
		io_libecc_Env.printColor(0, io_libecc_env_bold, "all success");
	
	io_libecc_Env.newline();
	io_libecc_Env.newline();
	io_libecc_Env.print("%d tests, %.2f ms", testCount, testTime * 1000);
