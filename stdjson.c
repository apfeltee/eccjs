...
const eccobjinterntype_t io_libecc_json_type = {
...
static int stringifyValue(eccjsonstringify_t* stringify, eccvalue_t thisval, eccvalue_t property, eccvalue_t value, int isArray, int addComa)
{
    ...
    if(!isArray)
        io_libecc_Chars.append(&stringify->chars, "\"%.*s\":%s", ECCNSValue.stringLength(&property), ECCNSValue.stringBytes(&property),
